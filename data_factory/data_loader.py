...
class WTB(object):
    def __init__(self, data_path, win_size, step, mode="train"):
        ...
        self.test_labels = np.load(data_path + '/label.npy')
...
def get_loader_segment(data_path, batch_size, win_size=100, step=100, mode='train', dataset='PSM'):
    if dataset == 'SMD':
        ...
        dataset = WADI(data_path, win_size, step, mode)
    elif dataset == 'WTB':
        dataset = WTB(data_path, win_size, step, mode)
    elif dataset == 'WTF':
        dataset = WTF(data_path, win_size, step, mode)
    elif dataset == 'SWaT':
        dataset = SWaT(data_path, win_size, step, mode)
    shuffle = False
    if mode == 'train':
        shuffle = True
